...
//========================================================================================//
//========================================================================================//
#define TEXCOFFV(nind,xt,yt,zt,I,h) ArrcoffV[nind] = tex3D(layerRefV, (zt)*texStretch[0].z+texShift[0].z, (yt+iy*NDT*2)*texStretch[0].y+texShift[0].y, GLOBAL(xt)*texStretch[0].x+texShift[0].x);
#define TEXCOFFT(nind,xt,yt,zt,I,h) ArrcoffT[nind] = tex3D(layerRefT, (zt)*texStretch[0].z+texShift[0].z, (yt+iy*NDT*2)*texStretch[0].y+texShift[0].y, GLOBAL(xt)*texStretch[0].x+texShift[0].x);
#define TEXCOFFS(nind,xt,yt,zt,I,h) ArrcoffS[nind] = tex3D(layerRefS, (zt)*texStretch[0].z+texShift[0].z, (yt+iy*NDT*2)*texStretch[0].y+texShift[0].y, GLOBAL(xt)*texStretch[0].x+texShift[0].x);

const int CntX=Np*2*NDT/2;
const int CntY=Na*NasyncNodes*2*NDT/2;
const int CntZ=Nz*2/2;
#define DIST(xt,yt,zt) sqrtf((GLOBAL(xt)-CntX)*(GLOBAL(xt)-CntX)+(yt+phys_iy*NDT*2-CntY)*(yt+phys_iy*NDT*2-CntY)+(zt-CntZ)*(zt-CntZ))
#define TEXCOFFV(nind,xt,yt,zt,I,h) ArrcoffV[nind] = tex3D(layerRefV, DIST(xt,yt,zt)*texStretch[0].z+texShift[0].z, 0,0);
#define TEXCOFFT(nind,xt,yt,zt,I,h) ArrcoffT[nind] = tex3D(layerRefT, DIST(xt,yt,zt)*texStretch[0].z+texShift[0].z, 0,0);
#define TEXCOFFS(nind,xt,yt,zt,I,h) ArrcoffS[nind] = tex3D(layerRefS, DIST(xt,yt,zt)*texStretch[0].z+texShift[0].z, 0,0);
//#define TEXCOFFV(nind,xt,yt,zt,I,h) ArrcoffV[nind] = float(*((double*)(&(h))));/* ArrSurf[nind] = surf; */
//#define TEXCOFFT(nind,xt,yt,zt,I,h) ArrcoffT[nind] = float(*((double*)(&(h))));
#//define TEXCOFFS(nind,xt,yt,zt,I,h) ArrcoffS[nind] = *((float2*)(&(h)));
#define TEXCOFFTx(nind,xt,yt,zt,I,h) TEXCOFFT(nind,xt,yt,zt,I,h)
#define TEXCOFFTy(nind,xt,yt,zt,I,h) TEXCOFFT(nind,xt,yt,zt,I,h)
#define TEXCOFFTz(nind,xt,yt,zt,I,h) TEXCOFFT(nind,xt,yt,zt,I,h)
//#define TEXCOFFQ(nind,xt,yt,zt,I,Q) ArrcoffQ[nind] = make_ftype2(Q,Q);
#define TEXCOFFQ(nind,xt,yt,zt,I,Q) ArrcoffQ[nind] = tex3D(layerRefQ, DIST(xt,yt,zt)*texStretch[0].z+texShift[0].z, 0,0);
...
#define REG_DEC(EVENTYPE) \
  ...
  ftype2 coffQ = make_ftype2(0,0); ftype2 ArrcoffQ[100];\
  register ftype coffVdef = defCoff::drho, coffTdef=defCoff::Vs*defCoff::Vs*defCoff::rho;\
  register coffS_t coffSdef = make_ftype2(defCoff::Vp*defCoff::Vp*defCoff::rho, (defCoff::Vp*defCoff::Vp-2*defCoff::Vs*defCoff::Vs)*defCoff::rho);\
  ftype2 dCellVols;\
  float alpha,beta,afloor,bfloor,bceil;\
  const int texNwindow = int(ceil(Ns*NDT*2.0*texStretch[0].x)+2);\
  int I; register htype h[100]; float2 q[100]; int Ind;\
